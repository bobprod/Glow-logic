...
import { FlaskConical, Film, Image as ImageIcon, Loader2, Sparkles } from "lucide-react";
    ...
    <div className="border-t border-white/5 pt-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-white text-xs font-black uppercase tracking-widest flex items-center gap-2">
            ...
            IA Media
            <span
              title="Fonctionnalite experimentale : aucune generation media reelle n'est cablee. Seul un storyboard textuel local est produit."
              className="flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-amber-300 bg-amber-500/10 border border-amber-500/30 px-2 py-0.5 rounded-full"
            >
          ...
          <p className="text-[11px] text-slate-500 mt-1 font-semibold">
            Storyboard textuel local uniquement. Aucune generation video/image reelle (fournisseur non cable).
          </p>
        </div>
      ...
      <div className="grid grid-cols-[1fr_120px] gap-2">
        ...
        <button
          onClick={generate}
          disabled={isBusy}
          title="Genere uniquement un storyboard textuel local. La generation video/image reelle n'est pas cablee."
          className="min-h-[42px] rounded-xl bg-amber-400/80 hover:bg-amber-300 text-black text-xs font-black transition-all flex items-center justify-center gap-2 disabled:opacity-60"
        >
