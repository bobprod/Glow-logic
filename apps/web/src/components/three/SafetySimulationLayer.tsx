...
function DroneModel({ position }: { position: Point3 }) {
  ...
  useFrame(({ clock }) => {
    ...
    // Hover effect
    droneRef.current.position.y = position[1] + Math.sin(t * 2) * 0.1;
  });
...
function DroneSimulation() {
  ...
  const dronePosition = useRef<Point3>(safePath[0] || [0, 0, 0]);
  ...
  useFrame(({ clock }) => {
    ...
    dronePosition.current = [droneRef.current.position.x, droneRef.current.position.y, droneRef.current.position.z];
  });
  ...
  return (
    <group>
      ...
      <DroneModel position={dronePosition.current} />
    </group>
  );
}
...
function PyroSimulation({ armed }: { armed: boolean }) {
  ...
  return (
    <group position={[-6.5, 0.08, 5.8]}>
      ...
      <PyroExplosion position={[-6.5, 1.5, 5.8]} armed={armed} />
    </group>
  );
}
