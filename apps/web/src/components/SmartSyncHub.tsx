...
export default function SmartSyncHub() {
  ...
  // Floating reticle tracking animation
  const [reticlePos, setReticlePos] = useState({ x: 45, y: 40 });
  useEffect(() => {
    if (!smartAutoPilot) return;
  ...
  // AI Show Generator (Music analysis & timeline generation)
  const handleGenerateShow = async () => {
    setIsAnalyzing(true);
    addToast({
      type: 'info',
      message: 'Génération de show IA lancée',
      detail: 'Analyse du fichier MP3 et création automatique d\'automations',
      duration: 2000
    });
    ...
    setTimeout(() => {
      socket.emit('timeline_log', { type: 'info', text: "[AI] Calcul du tempo : 128.5 BPM détecté." });
    }, 500);

    setTimeout(() => {
      socket.emit('timeline_log', { type: 'ai', text: "[AI] Structure identifiée : Intro (0s-20s), Chorus (20s-60s), Drop (60s-120s), Break (120s-150s)." });
    }, 1200);
    ...
    setTimeout(() => {
      ...
      // Generate new clips on track 'fx' only if Safety Gate allows simulated laser content.
      validateSafetyAction({
        hazard: 'laser',
        outputMode: 'simulation',
        source: 'ai',
        description: 'AI show generator: LASER WAVES timeline clip',
      }).then((safety) => {
        if (safety.allowed) {
          ...
          addClip({ id: 'gen-c7-safe', track: 'fx', name: 'BEAM WAVES SAFE', startTime: 60000, duration: 60000, color: 'bg-green-500', textColor: 'text-green-400', qlcPage: 1, qlcWidget: 13 });
          addToast({
            type: 'warning',
            message: 'Safety Gate',
      ...
      socket.emit('timeline_log', { type: 'sys', text: "[AI] Séquence de show DMX insérée dans la timeline globale." });
    }, 2000);
  };
        ...
        <div className="flex items-center gap-6">
          <div 
            onClick={() => {
              ...
              addToast({
                type: 'info',
                message: smartAutoPilot ? 'Autopilot Désactivé' : 'Autopilot Activé',
                detail: smartAutoPilot ? 'Contrôle manuel uniquement' : 'Mouvements & transitions gérés par l\'IA'
              });
            }}
        ...
        <div className="flex gap-4 overflow-x-auto custom-scrollbar pb-1">
          ...
          >
            ...
            <span className="text-xs font-black w-2/3 text-left leading-tight uppercase tracking-wider">
              {isAnalyzing ? 'Génération...' : 'Générer Show IA (MP3)'}
            </span>
          </button>
