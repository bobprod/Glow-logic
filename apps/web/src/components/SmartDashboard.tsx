...
export default function SmartDashboard() {
  ...
  const {
    ...
    setSmartActiveScene: setActiveScene,
  ...
  // Socket connection status for Live Context — init false to avoid hydration mismatch
  ...
  const handleZoneChange = useCallback(
    (zoneName: string, rawValue: number) => {
  ...
  const handlePadClick = useCallback(
    ...
    [activeScene, pads, setActiveScene, addToast],
  );
            ...
            {Object.keys(ZONE_MAP).map((zone, i) => (
              <div key={zone} className="flex flex-col gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleZoneChange(zone, Number(e.target.value))
                  }
                />
              </div>
            ))}
          ...
          <div className="relative z-10 space-y-3">
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="text-white font-mono font-bold truncate max-w-[120px]">
                {activeScene !== null
                  ? (pads.find((p) => p.id === activeScene)?.name ?? "—")
                  : "—"}
              </span>
            </div>
            <div className="bg-black/50 rounded-lg p-3 border border-white/5 mt-4">
              ...
              <p className="text-xs text-slate-300 leading-relaxed">
                Cliquez sur une scène pour la déclencher. Passez en mode LIVE
                pour jouer aux pads.
              </p>
            </div>
          </div>
          ...
          {pads.map((pad) => {
            ...
            return (
              <div
                ...
                className={`rounded-2xl relative overflow-hidden cursor-pointer transition-all duration-200 hover:scale-[1.02] border-2 min-h-[140px] ${
                  isActive
              ...
              >
                ...
                <div className="relative h-full flex flex-col justify-end p-4">
                  <div
                    className={`mb-3 p-2.5 rounded-xl bg-white/10 w-fit ${pad.textColor}`}
                  >
                  ...
                  <p
                    className={`text-xs mt-1 font-medium ${isActive ? pad.textColor : "text-gray-500"}`}
                  >
