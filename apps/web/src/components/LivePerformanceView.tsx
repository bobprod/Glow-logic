...
export default function LivePerformanceView() {
    const { smartPads, bpm, midiLearnMode, midiLearnActiveControl, setMidiLearnActiveControl, midiMappings } = useStore();
    ...
    const isCrossfaderLearning = midiLearnMode && midiLearnActiveControl === crossfaderControlId;
    const hasCrossfaderMapping = !!midiMappings[crossfaderControlId];

    const handleCrossfaderClick = () => {
        if (midiLearnMode) {
            setMidiLearnActiveControl(isCrossfaderLearning ? null : crossfaderControlId);
        }
    };
    ...
    return (
        <div className="w-full h-full flex flex-col bg-[#0a0c10] overflow-hidden">
            ...
            <div className="flex-1 p-6 grid grid-cols-[1fr_auto_1fr] gap-8 overflow-hidden h-full">
                ...
                <div className="w-[340px] flex flex-col gap-6">
                    ...
                    <div className="h-28 bg-[#1a1c23] border-2 border-[#262c36] rounded-2xl flex items-center justify-center gap-4 relative shadow-xl overflow-hidden group">
                        ...
                        <span className="text-2xl text-slate-500 font-bold uppercase mt-6 select-none">BPM</span>
                        <button className="absolute right-4 top-4 text-slate-600 hover:text-cyan-400 transition-colors">
            ...
            <div className="absolute bottom-0 left-0 w-full h-24 bg-gradient-to-t from-[#0a0c10] via-[#0a0c10] to-transparent pointer-events-none flex items-end justify-center pb-6">
                ...
                >
                    {hasCrossfaderMapping && midiLearnMode && <div className="absolute -top-1 right-1/2 w-2 h-2 rounded-full bg-blue-400"></div>}
                    <span className="text-cyan-500 font-black text-xs tracking-widest w-8 text-center bg-cyan-500/10 rounded py-1">A</span>
                    <input
                        type="range"
                        min="0" max="100"
                        value={crossfader}
                    ...
                    />
                    <span className="text-purple-500 font-black text-xs tracking-widest w-8 text-center bg-purple-500/10 rounded py-1">B</span>
                </div>
            </div>
        </div>
    );
