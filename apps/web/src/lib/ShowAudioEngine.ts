...
class ShowAudioEngine {
  ...
  // --- Master track (timeline audio clock) — ADDITIF, séparé de l'analyse FFT/beat ---
  private masterTrackEl: HTMLAudioElement | null = null;
  private masterTrackLoaded = false;
  ...
  constructor() {
    if (typeof window !== 'undefined') {
      ...
      // Master track element (timeline audio clock) — parallèle, sans AudioContext.
      try {
        this.masterTrackEl = new Audio();
        this.masterTrackEl.preload = 'auto';
        this.masterTrackEl.addEventListener('ended', () => {
          if (this.onTrackEndedCallback) {
            try { this.onTrackEndedCallback(); } catch { /* noop */ }
          }
        });
        this.masterTrackEl.addEventListener('error', () => {
          this.masterTrackLoaded = false;
        });
      } catch {
        this.masterTrackEl = null;
        this.masterTrackLoaded = false;
      }
    }
  }
  ...
  // ============================================================================
  // MASTER AUDIO TRACK API (timeline audio clock) — ADDITIF & RÉTRO-COMPATIBLE.
  // Utilise un HTMLAudioElement dédié (masterTrackEl), indépendant de l'analyse
  // FFT/beat (audioEl). Aucune piste chargée => hasTrack()=false => l'appelant
  // garde son horloge delta. Robuste : un load échoué ne throw pas.
  // ============================================================================

  loadTrack(src: string): Promise<void> {
    return new Promise<void>((resolve) => {
      if (!this.masterTrackEl) {
        this.masterTrackLoaded = false;
        resolve();
        return;
      }

      const el = this.masterTrackEl;
      this.masterTrackLoaded = false;
      ...
      const onError = () => {
        this.masterTrackLoaded = false;
        cleanup();
        resolve(); // jamais de throw : état = pas de piste
      };
  ...
  playTrack(): void {
    if (!this.masterTrackEl || !this.masterTrackLoaded) return;
    try {
      const p = this.masterTrackEl.play();
      if (p && typeof p.catch === 'function') {
        p.catch((e) => console.warn('[ShowAudioEngine] playTrack failed', e));
      }
    } catch (e) {
      console.warn('[ShowAudioEngine] playTrack error', e);
    }
  }

  pauseTrack(): void {
    if (!this.masterTrackEl) return;
    try { this.masterTrackEl.pause(); } catch { /* noop */ }
  }

  stopTrack(): void {
    if (!this.masterTrackEl) return;
    try {
      this.masterTrackEl.pause();
      this.masterTrackEl.currentTime = 0;
    } catch { /* noop */ }
  }
  ...
  seekTrack(ms: number): void {
    ...
    try {
      ...
      this.masterTrackEl.currentTime = seconds;
    } catch { /* noop */ }
  }

  getAudioPositionMs(): number {
    if (!this.masterTrackEl || !this.masterTrackLoaded) return 0;
    const t = this.masterTrackEl.currentTime;
    return Number.isFinite(t) ? t * 1000 : 0;
  }

  getAudioDurationMs(): number {
    if (!this.masterTrackEl || !this.masterTrackLoaded) return 0;
    const d = this.masterTrackEl.duration;
    return Number.isFinite(d) && d > 0 ? d * 1000 : 0;
  }

  hasTrack(): boolean {
    return !!this.masterTrackEl && this.masterTrackLoaded;
  }

  isTrackPlaying(): boolean {
    if (!this.masterTrackEl || !this.masterTrackLoaded) return false;
    return !this.masterTrackEl.paused && !this.masterTrackEl.ended;
  }
  ...
  removeTrack(): void {
    ...
    try {
      this.masterTrackEl.pause();
      this.masterTrackEl.removeAttribute('src');
      this.masterTrackEl.load();
    } catch { /* noop */ }
    this.masterTrackLoaded = false;
  }
