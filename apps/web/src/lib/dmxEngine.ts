...
import { DmxOfflineQueue, type DmxOfflineQueueStatus } from './dmxOfflineQueue';
...
const DMX_QUEUE_STORAGE_KEY = 'glow-logic-dmx-offline-queue';
...
class DmxEngine {
    ...
    private offlineQueue = new DmxOfflineQueue();
    ...
    constructor() {
        if (typeof window !== 'undefined') {
            ...
            socket.on('connect', () => {
                console.log('[DmxEngine] Socket connected, flushing offline queue and forcing full resync');
    ...
    getOfflineQueueStatus(): DmxOfflineQueueStatus {
        return this.offlineQueue.status();
    }

    private dispatchQueueStatus() {
        if (typeof window === 'undefined') return;
        window.dispatchEvent(new CustomEvent('glowlogic:dmx-queue', { detail: this.getOfflineQueueStatus() }));
    }

    private persistOfflineQueue(force = false) {
        if (typeof window === 'undefined') return;
        const now = Date.now();
        if (!force && now - this.lastQueuePersistAt < 1000) return;
        this.lastQueuePersistAt = now;
        try {
            window.localStorage.setItem(DMX_QUEUE_STORAGE_KEY, JSON.stringify(this.offlineQueue.toSerialized()));
        } catch {
            // DMX live must not be interrupted by localStorage quota errors.
        }
    }

    private restoreOfflineQueue() {
        try {
            const raw = window.localStorage.getItem(DMX_QUEUE_STORAGE_KEY);
            if (!raw) return;
            this.offlineQueue.replaceFromSerialized(JSON.parse(raw));
            this.dispatchQueueStatus();
        } catch {
            // Ignore corrupt queue state.
        }
    }

    private queueOfflineWrite(universe: number, channel: number, value: number) {
        this.offlineQueue.queue(universe, channel, value);
        this.persistOfflineQueue();
        this.dispatchQueueStatus();
    }

    private flushOfflineQueue() {
        if (!socket.connected || this.offlineQueue.status().pending === 0) return;
        const writes = this.offlineQueue.drain();
        writes.forEach((write) => {
            socket.emit('dmx_update', {
                universe: write.universe,
                channel: write.channel,
                value: write.value,
            });
            if (!this.prev.has(write.universe)) this.prev.set(write.universe, new Map());
            this.prev.get(write.universe)!.set(write.channel, write.value);
        });
        this.persistOfflineQueue(true);
        this.dispatchQueueStatus();
    }
            ...
            chMap.forEach((value, channel) => {
                if (forced?.has(channel) || prevMap.get(channel) !== value) {
                    if (socket.connected) {
                        ...
                        this.queueOfflineWrite(universe, channel, value);
                    }
