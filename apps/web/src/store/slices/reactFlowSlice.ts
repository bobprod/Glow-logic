...
export const createReactFlowSlice: StateCreator<ReactFlowSlice, [], [], ReactFlowSlice> = (set, get) => ({
    ...
    addNode: (node: Node) => {
        ...
        const afterNodes = [...nodes, node];
        if (!isReplayingHistory()) {
            pushHistory({
                label: `Noeud ajoute: ${String(node.data?.label || node.id)}`,
                undo: () => set({ nodes, edges }),
                redo: () => set({ nodes: afterNodes, edges }),
            });
        }
        set({
            nodes: afterNodes,
    ...
    onNodesChange: (changes: NodeChange[]) => {
        ...
        if (hasRemove) {
            const { nodes, edges, past } = get();
            const afterNodes = applyNodeChanges(changes, nodes);
            const removedCount = nodes.length - afterNodes.length;
            if (!isReplayingHistory()) {
                pushHistory({
                    label: removedCount > 1 ? `${removedCount} noeuds supprimes` : 'Noeud supprime',
                    undo: () => set({ nodes, edges }),
                    redo: () => set({ nodes: afterNodes, edges }),
                });
            }
    ...
    onEdgesChange: (changes: EdgeChange[]) => {
        ...
        if (hasRemove) {
            const { nodes, edges, past } = get();
            const afterEdges = applyEdgeChanges(changes, edges);
            const removedCount = edges.length - afterEdges.length;
            if (!isReplayingHistory()) {
                pushHistory({
                    label: removedCount > 1 ? `${removedCount} liens supprimes` : 'Lien supprime',
                    undo: () => set({ nodes, edges }),
                    redo: () => set({ nodes, edges: afterEdges }),
                });
            }
            set({ past: [...past.slice(-49), { nodes: [...nodes], edges: [...edges] }], future: [] });
        }
    ...
    onConnect: (connection: Connection) => {
        ...
        if (!isReplayingHistory()) {
            pushHistory({
                label: 'Lien ajoute',
                undo: () => set({ nodes, edges }),
                redo: () => set({ nodes, edges: nextEdges }),
            });
        }
        set({
            edges: nextEdges,
