...
export const saveFixture = (
  ...
  if (id) {
    db.prepare(
      "UPDATE fixtures SET name=?, manufacturer=?, channels=?, total_channels=?, notes=?, start_address=?, modes=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
    ).run(name, manufacturer ?? null, jsonChannels, total, notes ?? null, addr, jsonModes, id);
    return id;
  }
  const result = db.prepare(
    "INSERT INTO fixtures (name, manufacturer, channels, total_channels, notes, start_address, modes) VALUES (?, ?, ?, ?, ?, ?, ?)",
  ).run(name, manufacturer ?? null, jsonChannels, total, notes ?? null, addr, jsonModes);
  return Number(result.lastInsertRowid);
};

export const getFixtures = (): FixtureListing[] => {
  const rows = db.prepare(
    "SELECT id, name, manufacturer, channels, total_channels, start_address, updated_at FROM fixtures ORDER BY updated_at DESC",
  ).all() as any[];
  return rows.map(r => {
    try {
