...
export interface SafetyState {
  ...
  armedExpiresAt: Record<HazardType, string | null>;
  rules: Array<{
    id: string;
    label: string;
    active: boolean;
...
// Cache court de l'etat safety: getSafetyState() est appele a chaque canal DMX
// (44 Hz x N canaux) et faisait autant de lectures SQLite synchrones bloquantes.
// Un TTL de 250 ms garde le hot-path DMX hors de SQLite tout en restant reactif.
const SAFETY_STATE_TTL_MS = 250;
let safetyStateCache: { value: SafetyState; expiresAt: number } | null = null;

function invalidateSafetyStateCache() {
  safetyStateCache = null;
}
...
function readArmedExpiresAt(hazard: HazardType) {
  const raw = getSetting(`safety_armed_until_${hazard}`);
  if (!raw) return null;
...
function expireArmedHazards() {
  const now = Date.now();
  for (const hazard of HAZARDS) {
    if (!readArmed(hazard)) continue;
    const expiresAt = Number(getSetting(`safety_armed_until_${hazard}`) || 0);
    if (Number.isFinite(expiresAt) && expiresAt > 0 && expiresAt <= now) {
      setSetting(`safety_armed_${hazard}`, "false");
      setSetting(`safety_armed_until_${hazard}`, "");
      addSupportLog("SAFETY", `${hazard} disarmed automatically after timeout`, "warning", { hazard });
    }
  }
}
...
export function getSafetyState(): SafetyState {
  const now = Date.now();
  if (safetyStateCache && safetyStateCache.expiresAt > now) {
    return safetyStateCache.value;
  }
  expireArmedHazards();
  ...
  const armedExpiresAt = Object.fromEntries(HAZARDS.map((hazard) => [hazard, readArmedExpiresAt(hazard)])) as Record<HazardType, string | null>;
  ...
  safetyStateCache = { value: state, expiresAt: now + SAFETY_STATE_TTL_MS };
  return state;
}
...
export function setHazardArmed(hazard: HazardType, armed: boolean, confirmation?: string) {
  ...
  setSetting(`safety_armed_until_${hazard}`, armed ? String(Date.now() + ARM_TTL_MS) : "");
  invalidateSafetyStateCache();
