...
import { startOscControl } from "./services/oscControl";
...
// CORS local explicite: accès tablette conserve via token, origines navigateur limitees.
...
function getDmxOutputStatus() {
  return outputHealth.getSnapshot(dmxRouter.getOutputs());
}

function emitDmxOutputStatus() {
  io.emit("dmx_output_status", getDmxOutputStatus());
}

outputHealth.onChange(emitDmxOutputStatus);

app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "glow-logic-api" });
});

app.post("/api/auth/bootstrap", (req, res) => {
  if (!isLoopback(req)) {
    addSupportLog("AUTH", "Bootstrap token refuse hors loopback", "warning", {
...
app.use("/api", (req, res, next) => {
  ...
  if (req.method === "POST" && req.path === "/auth/bootstrap") return next();
  return requireAuth(req, res, next);
});

app.get("/api/auth/token", (_req, res) => {
  const token = ensureApiToken();
  res.json({ token, masked: maskToken(token), allowedOrigins: getAllowedOrigins() });
});
...
app.post("/api/safety/role", (req, res) => {
  try {
    ...
    const state = setOperatorRole(role);
    io.emit("safety_status", {
      laserArmed: state.armed.laser,
      pyroArmed: state.armed.pyro,
      operatorRole: state.operatorRole,
      dangerousPhysicalOutputsEnabled: state.dangerousPhysicalOutputsEnabled,
...
app.post("/api/safety/arm", (req, res) => {
  try {
    ...
    const safetyState = setHazardArmed(selectedHazard, Boolean(armed ?? state), confirmation);
    io.emit("safety_status", {
      laserArmed: safetyState.armed.laser,
      pyroArmed: safetyState.armed.pyro,
      operatorRole: safetyState.operatorRole,
      dangerousPhysicalOutputsEnabled: safetyState.dangerousPhysicalOutputsEnabled,
    });
    res.json(safetyState);
  } catch (err: any) {
    addSupportLog("SAFETY", `Arm update failed: ${err.message}`, "warning");
    res.status(403).json({ error: "Armement impossible", details: err.message });
  }
});
...
async function handleShowActionsRequest(req: express.Request, res: express.Response) {
  try {
    ...
    if (result.results.some((actionResult) => actionResult.blocked)) {
      addSupportLog("SAFETY", "Show Action bloquee par Safety Gate", "warning", {
        ip: req.ip || req.socket.remoteAddress,
        route: req.path,
      });
...
app.post("/api/dmx/usb-config", (req, res) => {
  try {
    ...
    // Auto-sync router: enable serial outputs as soon as dongle is connected
    setTimeout(() => {
      const connected = usbDmx.getStatus().connected;
      dmxRouter.setOutputs({ usbDmx: connected, python: connected });
    }, 500);
...
// ── Patch table (re-patch logique→physique, univers 1) ────────
app.get("/api/patch", (_req, res) => {
  try {
    res.json({ spec: dmxRouter.getPatchSpec() });
  } catch (err) {
    res.status(500).json({ error: String(err) });
  }
});
...
app.post("/api/qlc/ws-enable", (_req, res) => {
  qlcWs.enable();
  dmxRouter.setOutputs({ qlcWs: true, qlcOsc: false, usbDmx: false, python: false });
...
app.get("/api/dmx/output-status", (_req, res) => {
  res.json(getDmxOutputStatus());
});

app.post("/api/dmx/router", (req, res) => {
  try {
    dmxRouter.setOutputs(req.body);
    const outputs = dmxRouter.getOutputs();
    res.json({ success: true, outputs, outputStatus: outputHealth.getSnapshot(outputs) });
  } catch (err) {
    res.status(500).json({ error: String(err) });
  }
});
...
app.get("/api/dmx/live", (_req, res) => {
  const universe = 1;
  res.json({ universe, channels: dmxRouter.getLiveUniverse(universe), timestamp: Date.now() });
});
...
io.on("connection", (socket) => {
  ...
  socket.emit("dmx_output_status", getDmxOutputStatus());
...
let dmxOutputHeartbeat: ReturnType<typeof setInterval> | null = null;
...
export function startServer(port = PORT) {
  ...
  httpServer.listen(port, "0.0.0.0", () => {
    if (!dmxOutputHeartbeat) {
      dmxOutputHeartbeat = setInterval(emitDmxOutputStatus, 5000);
      dmxOutputHeartbeat.unref?.();
    }
  ...
  // OSC bidirectional control surface (tablet: TouchOSC/Lemur) — additive, namespace /glow/*
  startOscControl(io);
  ...
  pythonDmx.on("ready", () => {
    dmxRouter.setOutputs({ python: true, usbDmx: false });
    outputHealth.reportConfigChange();
    io.emit("dmx_bridge_status", { ready: true });
  });
  pythonDmx.on("error", (err) => {
    console.error("⚠️ [Python DMX Bridge Error]", err);
    dmxRouter.setOutputs({ python: false });
    outputHealth.reportError("python", err);
  ...
  qlcEngine.on("claiming-device", () => {
    ...
    dmxRouter.setOutputs({ usbDmx: false, python: false });
  });
